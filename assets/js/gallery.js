...
document.querySelector("button#all-animals").onclick = () =>
  filter(...allSpecies);
...
filter = (...allSpecies) => {
  ...
  let animalSpecies = [];
  for (let species of allSpecies)
    animalSpecies = animalSpecies.concat(
      animalArray.filter((animal) => animal.species === species).sort()
    );
  for (let animal of animalSpecies) {
    ...
    img.src = `assets/images/${animal.name
      .toLowerCase()
      .replace(" ", "-")}.jpg`;
    img.alt = animal.name;
